...
GPU-safe: `hungarian_match` (above) does the CPU-only combinatorial matching inside
`Zygote.@ignore`, returning plain `Int` index vectors -- cheap to keep on the CPU, they're
just used for indexing. Everything that needs a gradient (`rec_sum`, `ℒ_exist`) is
computed by indexing the *original* `x̂`/`x`/`logits_exist` (wherever they actually live,
CPU or GPU), never the CPU copies made for matching. The one thing that has to be built
fresh and device-matched is `exist_target`, since it's compared against `logits_exist`
via `logitbinarycrossentropy` and Flux/CUDA won't broadcast a plain CPU Array against a
CuArray.
"""
function hungarian_matching_loss(x̂::AbstractArray{T,3}, logits_exist::AbstractArray{T,3}, x::AbstractArray{T,3}, x_mask::AbstractArray{Bool,3}) where T<:AbstractFloat
    ...
    matched_pred, matched_gt, exist_target = Zygote.@ignore begin
        ...
        t = x̂ isa CUDA.CuArray ? CUDA.cu(t) : t # match exist_target's device to x̂/logits_exist
        mp, mg, t
    end
    ...
    for b in 1:bs
        ...
        isempty(pi) && continue
        rec_sum = rec_sum + sum(abs2, x̂[:, pi, b] .- x[:, gi, b]) # x̂, x stay on their original device -- fancy-indexed with plain Int vectors, which CUDA.jl supports (gather, not scalar indexing)
        n_matched += length(pi)
    end
...
for epoch in 1:args.epochs
    ...
    for batch in tqdm(dataloaders.train)
        ...
        model, opt, logs = optim_step(model, batch, opt, device; β=args.β, λ_exist=args.λ_exist)
    end
    vlogs, _ = valid_step(model, dataloaders.valid, device; β=args.β, λ_exist=args.λ_exist)
...
x̂_gen, logits_gen = generate(model, 200, device)
x̂_gen, logits_gen = cpu(x̂_gen), cpu(logits_gen)
