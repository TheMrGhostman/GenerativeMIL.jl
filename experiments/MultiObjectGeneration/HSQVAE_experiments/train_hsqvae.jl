...
function main()
    ...
    @add_arg_table! s begin
        "config_file"
            arg_type = String
            default = joinpath(@__DIR__, "configs", "test_configs", "cd_hsqvae_test_cfg.yml")
    ...
    train_cfg = Dict{Symbol,Any}(cfg[:train]) #TODO: check this
    ...
    if !isdir(train_cfg[:model_dir])
        train_cfg[:model_dir] = datadir("HGenExperiments", "$(data_cfg[:dataset])", model_cfg[:model_type], "seed=$(args[:seed])", "$(train_cfg[:model_dir])_ID-$(lpad_number(args[:ui], Int(1e5)))" ) 
    end
    ...
    loss_cfg = get(train_cfg, :loss_function, "chamfer_pairwise_distance") 
